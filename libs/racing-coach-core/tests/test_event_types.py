...
class TestEventType:
    ...
    def test_create_event_type(self):
        ...
        assert event_type.data_type is str
    ...
    def test_event_type_frozen(self):
        ...
        with pytest.raises(Exception):  # dataclass frozen=True raises FrozenInstanceError
            event_type.name = "CHANGED"  # pyright: ignore[reportAttributeAccessIssue]
    ...
    def test_event_type_with_complex_data_type(self):
        ...
        event_type = EventType[dict[Any, Any]](name="COMPLEX_EVENT", data_type=dict)
        assert event_type.name == "COMPLEX_EVENT"
        assert event_type.data_type is dict
...
class TestEvent:
    ...
    def test_event_with_complex_data(self):
        ...
        event_type = EventType[dict[str, Any]](name="COMPLEX", data_type=dict)
        complex_data: dict[str, Any] = {"key1": "value1", "key2": [1, 2, 3]}
        event = Event(type=event_type, data=complex_data)
        assert event.data == complex_data

    def test_event_with_system_event_type(
        self, telemetry_and_session_factory: TelemetryAndSessionFactory
    ):
        """Test creating an Event with a system event type."""
        # Create an instance from the factory
        telemetry_data = telemetry_and_session_factory.build()
...
class TestHandlerContext:
    ...
    def test_create_handler_context(self, event_bus: EventBus, sample_event: Event[str]):
    ...
    def test_handler_context_frozen(self, event_bus: EventBus, sample_event: Event[str]):
        """Test that HandlerContext is immutable (frozen)."""
        context = HandlerContext(event_bus=event_bus, event=sample_event)
        with pytest.raises(Exception):  # dataclass frozen=True raises FrozenInstanceError
            context.event = None  # type: ignore

    def test_handler_context_access_event_data(self, event_bus: EventBus, sample_event: Event[str]):
...
class TestHandler:
    ...
    def test_handler_callable(self, sample_event_type: EventType[str], event_bus: EventBus):
