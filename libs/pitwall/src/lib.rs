...
//!     let connection = Pitwall::open("/path/to/session.ibt", 1.0).await?;
...
/// #[tokio::main]
/// async fn main() -> pitwall::Result<()> {
///     let connection = Pitwall::open("session.ibt", 1.0).await?;
///     // Use connection...
///     Ok(())
/// }
/// ```
...
impl Pitwall {
    ...
    /// * `speed` - Playback speed multiplier (1.0 = normal speed, 2.0 = double speed, etc.)
    ///
    /// # Errors
    ///
    /// Returns an error if:
    ...
    /// # #[tokio::main]
    /// # async fn main() -> pitwall::Result<()> {
    /// let connection = Pitwall::open("race.ibt", 1.0).await?;
    /// # Ok(())
    /// # }
    /// ```
    pub async fn open<P: AsRef<std::path::Path>>(path: P, speed: f64) -> Result<ReplayConnection> {
        ReplayConnection::open(path, speed).await
    }
}
