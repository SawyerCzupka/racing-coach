...
from racing_coach_core.events.base import EventBus, SystemEvents
...
class TestTelemetryCollectorUnit:
    ...
    def test_initialization(self, event_bus: EventBus, mock_telemetry_source):
        """Test TelemetryCollector initializes correctly."""
        collector = TelemetryCollector(event_bus, mock_telemetry_source)
    ...
    def test_collect_session_frame(self, event_bus: EventBus, mock_telemetry_source):
        """Test collecting session metadata."""
        collector = TelemetryCollector(event_bus, mock_telemetry_source)

        session = collector.collect_session_frame()
    ...
    def test_collect_and_publish_telemetry_frame_without_session(
        self, event_bus: EventBus, mock_telemetry_source
    ):
    ...
    def test_start_when_already_running(self, event_bus: EventBus, mock_telemetry_source):
    ...
    def test_stop(self, event_bus: EventBus, mock_telemetry_source):
...
class TestTelemetryCollectorIntegration:
    ...
    async def test_collector_thread_lifecycle(
        self, running_event_bus: EventBus, mock_telemetry_source
    ):
    ...
    async def test_collector_publishes_telemetry_events(
        self, running_event_bus: EventBus, mock_telemetry_source, event_collector
    ):
        ...
        events = event_collector.get_events_of_type(SystemEvents.TELEMETRY_FRAME)
        assert len(events) > 0
...
class TestTelemetryCollectorWithRealIBT:
    ...
    async def test_session_frame_consistency(
        ...
        try:
            ...
            session_frames = [event.data.SessionFrame for event in events]
            first_session = session_frames[0]
